...
class BookContribution(models.Model):
    ...
    def clean(self):
        ...
        # the key does not match with the book key 10-XNgf-idBQ-10
        book_id = int(str(self.book_key).split('-')[0])
        book_saved_key = Book.objects.filter(id=book_id).values('key')
        print(book_saved_key)
        print(book_saved_key[0]["key"], self.book_key == book_saved_key[0]["key"])

        if self.book_key != 'b10-XNgf-idBQ-10':
            raise ValidationError(
